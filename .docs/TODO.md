...
- [x] `devin` — audit in same file "devin" section; sourced from `backend/internal/adapters/agent/devin/devin.go:1-22` (verbatim self-description confirms Claude Code compat layer, share hook installer with claudecode) + `devin.go:86-100` (launch argv `devin [--permission-mode X] -p <prompt>`) + `devin.go:135-154` (restore argv `devin [-pm X] -r <agentSessionId>`) + `devin.go:253-266` (permission mapping: Default→no flag, AcceptEdits/Auto→`--permission-mode auto`, BypassPermissions→`--permission-mode dangerous`) + `devin.go:125-130` (hooks installer delegated directly to claudecode.Plugin.GetAgentHooks → `~/.claude/settings.local.json`, picked up by Devin's `config-importers/.../claude` + `agent-ext/hooks/importers/claude` layers) + recursive `grep "os.Getenv\|os.LookupEnv" backend/internal/adapters/agent/devin/devin.go` → **zero matches** (only `os.UserHomeDir()` for binary-path candidates at `devin.go:191,215` and `os.Stat` at `devin.go:281-283`). Adapter is fully pass-through. **Upstream CLI: closed-source** — `gh api repos/CognitionAI/devin-cli/git/trees/main?recursive=1` returns only `.github/`, `README.md` (single line "Try Devin CLI: https://docs.devin.ai/cli"), and a release-manifest script. **No source.** Direct-fetched docs: `https://docs.devin.ai/cli` (no env vars named; only install command + What's-next cards) + `https://docs.devin.ai/cli/models` (verbatim-quoted — model selection via `--model`/`/model`/`"agent": {"model": "..."}` in `~/.config/devin/config.json`; **no environment-variable override**; "Adaptive" is internal model-router, not user-routable) + `https://docs.devin.ai/cli/essential-commands` (verbatim-quoted — auth is via `/login` slash command, **no `DEVIN_API_KEY`**) + `https://docs.devin.ai/llms.txt` (full docs index scanned — no `cli/configuration` page exists, WebFetch sees HTTP 404; no `cli/env-vars` path; only `api-reference/authentication.md` exists, which documents `api.devin.ai` API tokens, **not** CLI model-traffic routing). **Negative finding**: no `DEVIN_API_KEY`/`DEVIN_BASE_URL`/`OPENAI_API_BASE`/`ANTHROPIC_BASE_URL`/`DEVIN_MODEL` env knobs anywhere reachable; the WebSearch-shaped claim of inherited `ANTHROPIC_*` is hypothetical (Claude *config* compat ≠ SDK-env compat, per the adapter's own doc-comment). The only model-key knob (`devin --model` or `~/.config/devin/config.json`'s `agent.model`) takes **short names like `opus`, `sonnet`, `swe`, `codex`, `gemini`** — never vendor-side base-URL-ish configs. **`DEVIN_API_KEY` is documented only for the `api.devin.ai` REST API**, not for the CLI's model traffic (would not transparently Bifrost-route). Adapter: zero env-touches (pass-through). **No clean Bifrost route in v1**; adapter unchanged.
- [x] `droid` — audit in same file "droid" section (`.docs/research/opencode-env-audit.md:2403`); sourced from `backend/internal/adapters/agent/droid/droid.go:1-18` (verbatim self-description, no Claude Code compat layer so AO installs its own hooks into worktree-local `.factory/hooks.json`) + `droid.go:90-115` (launch argv `droid [--settings <path>] [--append-system-prompt[-file] <x>] [prompt]`) + `droid.go:135-156` (restore argv `droid [--settings <path>] -r <agentSessionId>`) + `droid.go:179-194` (permission mapping: Default→empty, AcceptEdits→`low`, Auto→`medium`, BypassPermissions→`high` — written to process-scoped `os.TempDir()/ao-droid-<sanitized-id>-settings.json` because interactive TUI has no per-launch permission flag, only `droid exec` does) + `droid.go:271` (sole env-touch in entire package: `os.Getenv("APPDATA")` for Windows binary resolution; recursive grep `grep "APPDATA\|os.Getenv\|os.LookupEnv" backend/internal/adapters/agent/droid/` → 1 match line 271 only) + `hooks.go:17-28` (constants: `droidSettingsDirName=".factory"`, `droidHooksFileName="hooks.json"`, `droidHookCommandPrefix="ao hooks droid "`, `droidHookTimeout=30`; managed events SessionStart/UserPromptSubmit/Stop/Notification/SessionEnd). Upstream docs direct-fetched 2026-07-03: `https://docs.factory.ai/cli` (no env vars named) + `https://docs.factory.ai/cli/configuration/settings.md` (settings file paths `~/.factory/settings.json` / `<project>/.factory/settings.local.json`, **no env-var schema**; legacy was `.droid.yaml`, current is `.factory/` files; `/settings` is interactive config) + `https://docs.factory.ai/reference/cli-reference` (**only env var documented is `FACTORY_API_KEY`** auth-only; **no `--base-url`/`--baseUrl`/`--api-base` flag in 28 documented flags**; closest URL-adjacent is `--header KEY:VALUE` MCP-only) + `https://docs.factory.ai/cli/byok/overview.md` (**critical finding** — verbatim `"apiKey supports environment variable references using ${VAR_NAME} syntax"`; three `provider` types `anthropic`/`openai`/`generic-chat-completion-api`; CLI commands `/model` + `/cost` + `/settings`) + `https://docs.factory.ai/cli/byok/openai-anthropic.md` (verbatim JSON with `baseUrl: "https://api.anthropic.com"` + `baseUrl: "https://api.openai.com/v1"`) + `https://docs.factory.ai/cli/byok/ollama.md` (`baseUrl: "http://localhost:11434/v1"` + provider `generic-chat-completion-api`; adjacent env var `OLLAMA_CONTEXT_LENGTH=32000` is Ollama-side metadata, not Droid/Factor config) + `.../google-gemini.md` (`baseUrl: "https://generativelanguage.googleapis.com/v1beta/"`, models `gemini-2.5-pro`/`gemini-1.5-pro`/`gemini-1.5-flash`) + `.../huggingface.md` (`baseUrl: "https://router.huggingface.co/v1"`, example `meta-llama/Llama-4-Scout-17B-16E-Instruct:fireworks-ai`) + `.../openrouter.md` (`baseUrl: "https://openrouter.ai/api/v1"`, example `openai/gpt-oss-20b`); sibling pages `baseten`/`deepinfra`/`fireworks`/`groq`/`lm-studio` all use the same `customModels[]` shape (10+ first-party per-provider docs pages). **Verdict: clean Bifrost route #6, cleanest yet found** — `~/.factory/settings.json` `customModels[]` is the BYOK Bifrost-seam (explicit JSON with `baseUrl` + `apiKey` + `${ENV_REF}` substitution + `provider` + `model` + `displayName` + `maxOutputTokens`), three documented provider types, user-overridable `baseUrl`, no env-var gymnastics needed (no shell expansion, no `custom:` prefix parser, no provider-family-specific schema dialects). Adapter: fully pass-through (single APPDATA Windows lookup). **No adapter change needed** — Bifrost side emits per-session file at `os.TempDir()/ao-droid-<sanitized-id>-bifrost.json` mirroring the adapter's existing permission-delivery file, passed via `--settings <path>` (user decision in Phase 3 policy: write to user-scope `~/.factory/settings.json` vs. per-session `os.TempDir()` file). `FACTORY_API_KEY` is not Bifrost-routable (Factory-cloud auth, separate from provider model traffic).
- [ ] `goose`, `grok`, `kimi`, `kiro`, `pi`, `qwen`, `agy`, `vibe` — to do.
