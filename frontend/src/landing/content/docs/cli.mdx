...
| Variable                            | Purpose                                                                              |
| ----------------------------------- | ------------------------------------------------------------------------------------ |
| `AO_HOME`                           | Override the `~/.ao` base directory for the entire AO footprint (state, registry, telemetry, Electron userData). Additive: unset keeps `$HOME/.ao` as the default. |
| `AO_DATA_DIR`                       | Override the durable state dir only (`$AO_HOME/data` by default). Wins over `AO_HOME`. |
